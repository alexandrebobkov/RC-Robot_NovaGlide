---
layout: default
title: "ESP32-C3 RC Robot | NovaGlide"
---

{{ page.title }}
================

A modular, remotely controlled 6V robot built around the ESP32‑C3 microcontroller module, featuring wireless control via ESP‑NOW, real‑time telemetry of system metrics and health status through MQTT and InfluxDB, and integration with a wide range of sensors through I2C. Its modular architecture provides flexibility and scalability for adding new subsystems as the project evolves. Hardware and software were developed in parallel, enabling tight optimization of the embedded platform to meet the system’s constrained performance and power requirements.

![NovaGlide Robot](assets/NovaGlide_cover_16-9_web.jpg)

NovaGlide brings together a remarkably broad range of engineering concepts, making it a rich applied‑systems project rather than just a remote‑controlled robot. Its design spans everything from power‑supply management and signal scaling to PWM motor control, embedded networking, and real‑time telemetry streaming. The system architecture leverages FreeRTOS for task scheduling and concurrency, incorporates foundational and nonlinear control‑system techniques for smooth and responsive motion, and applies Human‑Machine Interaction principles to create intuitive steering behavior. This combination of disciplines makes NovaGlide a comprehensive demonstration of modern embedded‑systems engineering.

To make the system easier to understand and navigate, NovaGlide is organized around a clean architecture of independent subsystems. Each subsystem represents a distinct fusion of hardware and software, responsible for delivering a specific piece of functionality to the robot.

NovaGlide’s software is built around a deliberately modular, subsystem‑based architecture. Instead of concentrating all logic into a single monolithic control loop, each functional block is implemented as its own independent subsystem: the joystick HAL, steering mixer, motor‑control layer, telemetry pipeline, FreeRTOS scheduler, safety and fault‑handling logic, sensor interfaces, and the networking stack for ESP‑NOW and MQTT. This structure reflects a clear design philosophy: hardware and software should be co‑designed, and every subsystem should encapsulate a single, well‑defined functionality. The result is a system that is clean, maintainable, and inherently scalable. Developers and readers can understand NovaGlide one subsystem at a time, without being overwhelmed by the complexity of the whole robot, while still appreciating how each module integrates into the larger embedded platform.

NovaGlide’s architecture embraces a subsystem‑oriented design model, where every module exposes a clear, well‑defined interface through its own struct, encapsulating both internal state and the operations that act on it. This mirrors the patterns used in embedded HALs and modern robotics frameworks, ensuring that each subsystem — joystick HAL, steering mixer, motor control, telemetry, scheduler, safety and fault‑handling logic, sensor interfaces, and the networking stack — behaves as an independent component with a clean API. This design reinforces the principle of co‑designing hardware and software, with each subsystem delivering a single, focused responsibility. The result is a clean and extensible system in which modules can evolve independently, be tested in isolation, and interact through explicit, intentional boundaries rather than implicit coupling.
...
## RC-Robot 🔑 Key Components

### Motors Subsystem
**Location:** `subsystems/motors/`

**Purpose:** Controls 4 DC motors using PWM (left/right, forward/reverse, spin left/right)
...
**GPIO Pins:**
- Motor 1 (Left-side Forward):  GPIO 6
- Motor 2 (Right-side Forward): GPIO 5
- Motor 3 (Left-side Reverse):  GPIO 4
- Motor 4 (Right-side Reverse): GPIO 7
...
- Data structure: `sensors_data_t` struct with x_axis, y_axis values
