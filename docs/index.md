...
To make the system easier to understand and navigate, NovaGlide is organized around a clean architecture of independent subsystems. Each subsystem represents a distinct fusion of hardware and software, responsible for delivering a specific piece of functionality to the robot. This modular separation allows both the reader and the developer to focus on one subsystem at a time, gaining a complete and coherent understanding of its design, purpose, and behavior without being overwhelmed by the complexity of the entire platform.

NovaGlide’s software is built around a deliberately modular, subsystem‑based architecture. Instead of concentrating all logic into a single monolithic control loop, each functional block is implemented as its own independent subsystem: the joystick HAL, steering mixer, motor‑control layer, telemetry pipeline, FreeRTOS scheduler, safety and fault‑handling logic, sensor interfaces, and the networking stack for ESP‑NOW and MQTT. This structure reflects a clear design philosophy: hardware and software should be co‑designed, and every subsystem should encapsulate a single, well‑defined responsibility. The result is a system that is clean, maintainable, and inherently scalable. Developers and readers can understand NovaGlide one subsystem at a time, without being overwhelmed by the complexity of the whole robot, while still appreciating how each module integrates into the larger embedded platform.
